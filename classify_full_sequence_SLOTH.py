...
sloth_instance = sloth(model, max_time, len(action_names), 0.3, 0.80, 2, action_dict_inv)
...
for lab in labels_list:
    ...
    for i in range(len(labels_tensor)):
        ...
        if which_model == 'both':
            # outputs = model(video1_window, video2_window, imu_window, batch_length)
            sloth_instance.classify([video1_window, video2_window, imu_window, batch_length])
            sloth_instance.detect()
        elif which_model == 'imu':
            sloth_instance.classify([imu_window, batch_length])
            # print(f'shape: {imu_window_new.shape}')
            # outputs = model(imu_window, batch_length)
        elif which_model == 'videos':
            sloth_instance.classify([video1_window, video2_window, imu_window, batch_length])
        ...
        # print(f'\n{i/30.0:.2f}: {sloth.gestures[-1]}\n')
        # output_labels[i] = predicted



    # Save output_labels to a file
    if not os.path.exists(os.path.join(base_path, 'realtime_results', f'{which_model}_SLOTH')):
        os.makedirs(os.path.join(base_path, 'realtime_results', f'{which_model}_SLOTH'))

    with open(os.path.join(base_path, 'realtime_results', f'{which_model}_SLOTH', f'{length}_predicted_labels_{which_model}.txt'), 'w') as f:
