...
saving_path = '/home/s2412003/Shared/JAIST_Cylinder/Segmented_Dataset'
...
for (imu_idx, imu_seq, imu_lab, seq_len), (vid1_idx, vid1_seq, vid1_lab), (vid2_idx, vid2_seq, vid2_lab) in zip(train_loader_imu, train_loader_video1, train_loader_video2):
    ...
    #check that everything after seq_len is zero
    assert torch.all(imu_seq[0, seq_len.item():, :] == 0)
    assert torch.all(vid1_seq[0, seq_len.item():, :, :, :] == 0)
    assert torch.all(vid2_seq[0, seq_len.item():, :, :, :] == 0)

    np.save(os.path.join(saving_path, which_trial, 'IMU', f'{imu_idx[0]}_{imu_lab[0]}_{seq_len.item()}_imu.npy'), imu_seq[0, :seq_len.item(), :].numpy())
    np.save(os.path.join(saving_path, which_trial, 'Video1', f'{vid1_idx[0]}_{vid1_lab[0]}_{seq_len.item()}_video1.npy'), vid1_seq[0, :seq_len.item(), :, :, :].numpy())
    np.save(os.path.join(saving_path, which_trial, 'Video2', f'{vid2_idx[0]}_{vid2_lab[0]}_{seq_len.item()}_video2.npy'), vid2_seq[0, :seq_len.item(), :, :, :].numpy())

    # print(imu_seq[0].shape)
    # print(imu_seq[0, seq_len.item():, :].shape)
    # print(imu_seq[0, seq_len.item():, :])
    # print(imu_lab[0])
    # print(vid1_seq[0, seq_len.item()-2, 0, :, :])
    # print(vid1_seq.dtype)
    # print(vid2_seq[0, :seq_len.item(), :, :, :].shape)

    # print(f'{vid1_seq[0, seq_len.item()-1, 0, :, :]=}')
    # print(f'{vid1_seq[0, seq_len.item(), 0, :, :]=}')

    # print(f'{vid2_seq[0, seq_len.item()-1, 0, :, :]=}')
    # print(f'{vid2_seq[0, seq_len.item(), 0, :, :]=}')
