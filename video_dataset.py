...
class VideoDataset(Dataset):
    ...
    def load_sequence(self, action_image_names):
        ...
        sequence = self.pad_videos(sequence)
        # print(f'{sequence.shape=}')
        return sequence  # Shape: (C, T, H, W)


    def pad_videos(self, video):
        if video.shape[1] <= self.max_length:
            zeros = torch.zeros(video.shape[0], self.max_length, video.shape[2], video.shape[3])
